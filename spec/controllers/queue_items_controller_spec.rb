...
describe QueueItemsController do
  ...
  describe "POST update queue" do
    context "with valid inputs" do
      ...
      let(:queue_item1) { Fabricate(:queue_item, user: alex, position: 1, video: video) }
      let(:queue_item2) { Fabricate(:queue_item, user: alex, position: 2, video: video) }
    ...
    context "with queue items that do not belong to the current user" do
      it "does not change the queue items" do
        ...
        video = Fabricate(:video)
        bey = Fabricate(:user)
        queue_item1 = Fabricate(:queue_item, user: bey, position: 1, video: video)
        queue_item2 = Fabricate(:queue_item, user: alex, position: 2, video: video)
